  ...
  for (const raw of lines) {
    ...
    if (content.startsWith("- ")) {
      ...
      if (frame.lastKey) {
        ...
        if (Array.isArray(existing)) (existing as string[]).push(itemValue);
        else parent[frame.lastKey] = [itemValue];
      }
...
export default function LiveValidator({
  ...
  const statusLabel = useMemo(() => {
    ...
    return validation.hasSignature ? "Trust Chain: Signature Present" : "Trust Chain: Signature Missing";
  }, [validation]);
  ...
  useEffect(() => {
    if (propContent) processContent(propContent, propFileName || "live-builder.aix");
  ...
  const sigState = validation?.hasSignature ? "valid-structure" : validation ? "missing" : "unknown";
    ...
    <div className="rounded-2xl border border-[var(--color-glass-border)] bg-[rgba(12,16,28,0.5)] p-5 backdrop-blur-xl">
      ...
      <div
        ...
        className={`rounded-xl border-2 border-dashed p-6 text-center transition ${
          dragging ? "border-cyan-400 bg-cyan-500/10" : "border-[var(--color-glass-border)] hover:border-white/20"
        }`}
      >
      ...
      {fileName && <p className="mt-4 text-xs text-gray-400 truncate" title={fileName}>File: {fileName}</p>}
      {hash && <p className="mt-2 text-[10px] font-mono break-all text-cyan-200/80">SHA-256: {hash}</p>}
      {validation && (
